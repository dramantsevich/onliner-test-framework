...
Feature: Onliner feature
 ...
 Scenario: user login into onliner and get to cart items
    Given user navigates to "https://www.onliner.by/"
    When user is on home page
    When user click on "Sign_In" button
    And user enter "dzmitry.ramantsevich@expert-soft.by" email on Login page
    And user enter "5716294qD" password on Login page
    ...
    And user click on "Смартфон HONOR 30 BMH-AN10 8GB/128GB (изумрудно-зеленый)" Mobile Phone on Mobile Phones Catalog
    ...
    When user enter "Xiaomi Redmi 9A 2GB/32GB международная версия (серый)" on Search Input
    And user click on "Xiaomi Redmi 9A 2GB/32GB международная версия (серый)" Item on Search window
    Then user see Heading on "Product" page
    When user click on "Предложений" button
    And user click on "1" Offers list item on Product page
    Then user see Buy Button is green on Product Page
